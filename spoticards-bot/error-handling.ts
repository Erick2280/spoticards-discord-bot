...
export function returnUserFriendlyErrorMessage(error: Error, message: Message, server: Server, client: Client) {
    switch (error.message) {
        case 'Not Found':
            message.reply('não consegui criar um deck. Você tem certeza de que inseriu uma playlist válida do Spotify? Para trocar a playlist, use o comando **.changeplaylist <playlist>**.');
            break;
        ...
        case 'NotEnoughPlayersToStartGame':
            message.reply(`não há jogadores suficientes para começar o jogo. Outras pessoas podem entrar nele usando **.join** ${server.findGameIdentifierByUser(message.author.id)}.`);
            break;
        case 'GlobalDeckTooSmall':
            const game = server.findGameByIdentifier(server.findGameIdentifierByUser(message.author.id));
            message.reply(`a playlist fornecida é muito pequena para este jogo. Por favor, forneça uma playlist com pelo menos ${game.rounds * game.players.length} músicas ou jogue com menos rodadas ou jogadores.`);
            break;
        ...
        case 'PlayerCardAlreadyUsed':
            message.reply('você já jogou essa carta. Jogue uma que não tenha jogado antes :)');
            break;
...
export function returnExpectedCommandUsage(prefix: string, commandName: string, usage: string, message: Message) {
    message.reply(`para usar este comando, você precisa enviar uma mensagem assim: **${prefix + commandName + ' ' + usage}**.`);
}